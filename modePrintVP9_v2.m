...
for modeIndx = 1:length(mode)
    ...
    for tuIndex = 1:length(g_tu_x_y)
        ...
        if(uiDirMode == 1)
            for j  =0:iHeight-1
                for i= 0:iWidth-1
                    ...
                    if((j == 0) && (i~= iHeight-1))
                       ...
                       iXn = i+1;
                       iXnP2 = i+1;
                    elseif((j == 1) &&(i~=iHeight-1) &&(i~=iHeight-2))
                       iXnN1 = i;
                       iX = i+1;
                       iXn = i+2;
                       iXnP2 = i+2;
                    elseif((j == 1) &&(i==iHeight-2))
                       iXnN1 = iHeight -1;
                       iX = iHeight - 2;
                       iXn = iHeight -2;
                       iXnP2 = iHeight -2;
            ...
            for j  =2:iHeight-1
                for i= iHeight-2:-1:0
                   iXnN1 = calcPara.computeMatri{j-2 +1,i+1 +1}(1);
                   iX = calcPara.computeMatri{j-2 +1,i+1 +1}(2);
                   iXn = calcPara.computeMatri{j-2 +1,i+1 +1}(3);
                   iXnP2 = calcPara.computeMatri{j-2 +1,i+1 +1}(4);
                   calcPara = saveAllPoints(allFid, uiDirMode, iWidth, iHeight, i, j, iXnN1, iX, iXn, iXnP2,1, 1, calcPara, calcAll);
                end
            end  
        end
